    ...
    return (
        <div>
            ...
            <div className="space-y-4">
                {experience.map(exp => (
                    <div key={exp._id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between hover:shadow-md transition-shadow">
                        <div className="flex-1 pr-0 md:pr-6">
                            <h2 className="text-lg md:text-xl font-bold text-gray-800">{exp.role} <span className="text-gray-400 font-normal">at</span> {exp.company}</h2>
                            <p className="text-indigo-600 font-medium mb-3 mt-1 text-sm">{exp.duration}</p>
                            <p className="text-sm md:text-base text-gray-600 whitespace-pre-wrap">{exp.description}</p>
                        </div>
