            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                {messages.length === 0 ? (
                    ...
                    <ul className="divide-y divide-gray-200">
                        {messages.map(msg => (
                            <li key={msg._id} className={`p-4 md:p-6 transition-colors ${!msg.isRead ? 'bg-indigo-50 hover:bg-indigo-100/50' : 'bg-white hover:bg-gray-50'}`}>
                                <div className="flex justify-between items-start">
                                    <div className="flex items-start flex-1 min-w-0 pr-4 md:pr-6">
                                        <button onClick={() => !msg.isRead && markAsRead(msg._id)} className={`mr-3 md:mr-4 mt-1 transition-colors ${!msg.isRead ? 'text-indigo-600 hover:text-indigo-800' : 'text-gray-300'}`} title={!msg.isRead ? "Mark as read" : "Read"}>
                                            {!msg.isRead ? <FaCircle /> : <FaCheckCircle className="text-xl" />}
                                        </button>
                                        <div className="min-w-0">
                                            <h3 className="font-bold text-gray-900 truncate">
                                                {msg.name} 
                                                <span className="block md:inline font-normal text-gray-500 md:ml-2 truncate">&lt;{msg.email}&gt;</span>
                                            </h3>
                                            <p className="text-xs text-gray-500 mb-3">{new Date(msg.createdAt).toLocaleString()}</p>
                                            <p className="text-sm md:text-base text-gray-700 whitespace-pre-wrap break-words">{msg.message}</p>
                                        </div>
                                    </div>
